...
func NewSubjectRepositoryImpl(db *sql.DB) *SubjectRepositoryImpl {
...
func (repo *SubjectRepositoryImpl) Save(subject entities.Subject) error {
    ...
    _, err := repo.db.Exec(query, subject.Name, subject.Credit)
    return err
}
...
func (repo *SubjectRepositoryImpl) Update(id int, subject entities.Subject) error {
    ...
    _, err := repo.db.Exec(query, subject.Name, subject.Credit, id)
    return err
}
